...
def focal_loss(
    ...
    *,
    ...
    """
    Focal loss with focusing parameter gamma and class weights alpha (inverse frequency).
    If alpha is None, uniform weights are used.
    """
    if alpha is None:
        alpha = [1.0] * logits.size(1)
    alpha_t = torch.tensor(alpha, device=logits.device)[target]
...
def train_gru(
    *,
    ...
    """
    Train a GRU model using focal loss, AdamW optimizer, ReduceLROnPlateau on validation F1,
    and early stopping based on F1.
    """
