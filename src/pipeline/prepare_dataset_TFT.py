...
class TFTDataset(Dataset):
    """
    Sliding-window Dataset for Temporal Fusion Transformer.
    Each item: (X_window, y_target)
        X_window: Tensor [seq_len, D]
        y_target: scalar tensor (float/int)
    """
...
def prepare_tft_dataset(
    ...
    *,
    ...
    df_train.loc[:, continuous_cols] = scaler.fit_transform(
        df_train[continuous_cols].astype("float32")
    )
    df_test.loc[:, continuous_cols] = scaler.transform(
        df_test[continuous_cols].astype("float32")
    )
