...
def prepare_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1) 5-минутные SMA/STD для объёма и доходности (10 свечей)
    close = df_feat['ohlcv_5m_close']
    vol = df_feat['ohlcv_5m_vol']
    ret = close.pct_change()

    df_feat['vol_ma_5_10'] = vol.rolling(10, min_periods=1).mean()
    df_feat['vol_std_5_10'] = vol.rolling(10, min_periods=1).std(ddof=0)
    df_feat['ret_ma_5_10'] = ret.rolling(10, min_periods=1).mean()
    df_feat['ret_std_5_10'] = ret.rolling(10, min_periods=1).std(ddof=0)
    # дополнительное сглаживание импульса
    df_feat['ret_ewm_5m'] = ret.ewm(span=10, adjust=False).mean()

    # 2) 15-мин ресемплинг: EMA(20)+наклон, RSI14, объём, волатильность
    df15 = pd.DataFrame({
        'close_15m': close.resample('15min').last(),
        'vol_15m': vol.resample('15min').sum(),
        'ret_15m': ret.resample('15min').std(ddof=0),
    })
    ...
    # 3) 30-мин ресемплинг: MACD(12,26,9), RSI14, объём, волатильность
    df30 = pd.DataFrame({
    ...
    # 4) 60-мин ресемплинг: EMA20 + наклон
    df60 = pd.DataFrame({
        'close_60m': close.resample('60min').last()
    })
    ...
    # 6) Флаги пропусков для реальных признаков
    real_feats = [
    ...
    # маска пропусков для cross-TF фичей
    for feat in ['ema_slope_acc', 'rsi_ratio_15_30']:
        df_merged[f'{feat}_na'] = df_merged[feat].isna().astype('int8')
    df_merged[['ema_slope_acc', 'rsi_ratio_15_30']] = df_merged[['ema_slope_acc', 'rsi_ratio_15_30']].fillna(0).astype(
        'float32')
...
def label_microtrend(
        ...
        window: int = 3,
        threshold: float = 0.001 # тут поправить в будущем
) -> pd.Series:
    """
    Разметка микротрендов 5M:
      +1 если (price.shift(-window)/price - 1) >= threshold
      -1 если <= -threshold
       0 иначе
    """
    close = df[price_col]
